...
| | [jipin-ai/agent-pipeline-engine](https://github.com/jipin-ai/agent-pipeline-engine) | ⭐⭐ | 想看「不信 agent 自報完成」怎麼落成程式碼 | 不到 400 行的多 agent 編排實作（Python 狀態機 + YAML 管道定義、零外部依賴）：任務交付必須附 git commit、測試報告、SHA256 三項證據，缺一項狀態機拒絕推進；另有審計鏈與人類介入點。對照 LangGraph 的 framework 路線，這條是「自己寫狀態機」的參照。**很新**：2026-08-10 建立、1 位 contributor，還沒有長期維護紀錄。★ 0、MIT |
